...
class DatasetFolderMultiTransform(DatasetFolder):
    ...
    def __getitem__(self, index):
        ...
        return (samples, target, path) if self.is_PSAD_eval==True else (samples, target)
...
class PSAD_single_dataset(ImageFolderMultiTransform):
    def __init__(self, root, transform=None, target_transform=None, loader=default_loader, is_valid_file=None, n_transforms=c.n_transforms):
        super().__init__(root, transform, target_transform, loader, is_valid_file, n_transforms)

class PSAD_multi_dataset(ImageFolderMultiTransform):
    def __init__(self, root, transform=None, target_transform=None, loader=default_loader, is_valid_file=None, n_transforms=c.n_transforms):
        super().__init__(root, transform, target_transform, loader, is_valid_file, n_transforms)
    ...
    def __getitem__(self, index):
        ...
        for path, target in samples_list:
            ...
            channel_concat_list.append(samples)

        samples = torch.cat(channel_concat_list,dim=1)
