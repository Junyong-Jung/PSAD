...
import config as c
from PSAD_dataset import PSAD_multi_dataset, PSAD_single_dataset
...
def load_datasets(dataset_path, class_name):
    ...
    if 'single' in plus_variable.version:        
        trainset = PSAD_single_dataset(data_dir_train, transform=transform_train, n_transforms=c.n_transforms)
        testset = PSAD_single_dataset(data_dir_test, transform=transform_train, target_transform=target_transform,
                                            n_transforms=c.n_transforms_test)
    elif 'multi' in plus_variable.version:
        trainset = PSAD_multi_dataset(data_dir_train, transform=transform_train, n_transforms=c.n_transforms)
        testset = PSAD_multi_dataset(data_dir_test, transform=transform_train, target_transform=target_transform,
                                            n_transforms=c.n_transforms_test)
...
def make_dataloaders(trainset, testset):
    trainloader = DataLoader(trainset, pin_memory=True, batch_size=c.batch_size,shuffle=True, num_workers=10,
                                              drop_last=False)
    testloader = DataLoader(testset, pin_memory=True, batch_size=c.batch_size_test, shuffle=True, num_workers=10,
                                             drop_last=False)
