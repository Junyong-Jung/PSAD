...
class CSA_module(nn.Module):
    def __init__(self, previous_conv_ch, num_imgs=5) -> None:
        super().__init__()
        assert previous_conv_ch is not None
        self.num_imgs = num_imgs
        MLP_ch = previous_conv_ch * num_imgs
        self.adaptive_avg = nn.AdaptiveAvgPool2d((1,1))

        self.channel_module = nn.Sequential(
                                            nn.ReLU(),
                                            nn.Linear(MLP_ch, MLP_ch // num_imgs, bias=False),
                                            nn.ReLU(),
                                            nn.Linear(MLP_ch // num_imgs, MLP_ch, bias=False),
                                            )
        self.softmax = nn.Softmax(dim=1)
        
    def forward(self, input):
        avgpooled = self.adaptive_avg(input)
        avgpooled = torch.flatten(avgpooled, start_dim=1)
        avgpooled = self.channel_module(avgpooled)
        channel_attention_weight = self.softmax(avgpooled)
        channel_attention_weight = channel_attention_weight.unsqueeze(-1).unsqueeze(-1)
        output = input * channel_attention_weight
        output = torch.split(output, output.shape[1]//self.num_imgs, dim=1)
        output = sum(output)
        return output
...
class PSAD_earlyfusion_BasicDiscriminator(BasicDiscriminator):
    def __init__(self, opt):
        super().__init__(opt)
        ...
        # input is nc x isize x isize
        feat.add_module('initial-conv-{0}-{1}'.format(nc, ndf),
                        nn.Conv2d(nc * plus_variable.num_imgs, ndf, 4, 2, 1, bias=False))
        feat.add_module('initial-relu-{0}'.format(ndf),
                        nn.LeakyReLU(0.2, inplace=True))
        csize, cndf = isize / 2, ndf
...
##
class PSAD_channel_attention_fusion_BasicDiscriminator(BasicDiscriminator):
    """
    NETD
    """
    def __init__(self, opt):
        ...
        feat = list(feat)
        CSA_layers = CSA_module(plus_variable.previous_conv_ch, num_imgs=plus_variable.num_imgs)
        feat.insert(plus_variable.insert_location, CSA_layers)
    ...
    def forward(self, input):
        if isinstance(input.data, torch.cuda.FloatTensor) and self.ngpu > 1:
            ...
            x_list = torch.split(input,3,dim=1)
            y_list = []
            for x in x_list:
                x = self.feat[:plus_variable.insert_location](x)
                y_list.append(x)
            x = torch.cat(y_list,dim=1)
            feat =  self.feat[plus_variable.insert_location:](x)
            clas = self.clas(feat)
        clas = clas.view(-1, 1).squeeze(1)
        return clas, feat
...
def define_G(opt, norm='batch', use_dropout=False, init_type='normal'):
    ...
    if 'single' in plus_variable.version or 'late' in plus_variable.version:
        ...
        netG = PSAD_earlyfusion_UnetGenerator(opt.nc, opt.nc, num_layer, opt.ngf, norm_layer=norm_layer, use_dropout=use_dropout)
    elif 'attention' in plus_variable.version:
        netG = PSAD_channel_attention_fusion_UnetGenerator(opt.nc, opt.nc, num_layer, opt.ngf, norm_layer=norm_layer, use_dropout=use_dropout)
...
def define_D(opt, norm='batch', use_sigmoid=False, init_type='normal'):
    ...
    if 'single' in plus_variable.version or 'late' in plus_variable.version:
        ...
        netD = PSAD_earlyfusion_BasicDiscriminator(opt)
    elif 'attention' in plus_variable.version:
        netD = PSAD_channel_attention_fusion_BasicDiscriminator(opt)
...
class PSAD_earlyfusion_UnetGenerator(UnetGenerator):
    def forward(self, input):
        x_list = torch.split(input,3,dim=1)
        y_list = []
        for x in x_list:
            x = self.model(x)
            y_list.append(x)
        output = torch.cat(y_list,dim=1)
        return output
    
    
    
class PSAD_channel_attention_fusion_UnetGenerator(UnetGenerator):
    def forward(self, input):
        x_list = torch.split(input,3,dim=1)
        y_list = []
        for x in x_list:
