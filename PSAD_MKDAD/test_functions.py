...
def gradients_localization(model, vgg, test_dataloader, config):
    ...
    for data in test_dataloader:
        ...
        if 'single' in config['version']:
            single_plot_heatmap(X.cpu().numpy().copy(), temp.copy(), index_path)
        elif 'multi' in config['version']:
            multi_plot_heatmap(X.cpu().numpy().copy(), temp.copy(), index_path)
        
    output = np.concatenate(full_list,axis=0)    
...
def single_plot_heatmap(test_img:np.ndarray, grad, index_path_list):
    ...
    for i in range(num):
        ...
        index_path = index_path_list[i].split(os.path.sep)
        category = index_path[7]
        index_path = "_".join(index_path[7:])
        index_path = os.path.splitext(index_path)[0]
        plt.axis('off')
        plt.imshow(img, interpolation='none')
        plt.savefig(os.path.join('heatmap_outputs',category,index_path+'_Orig_Image.png'),bbox_inches='tight', pad_inches=0, dpi=96)
        plt.clf()
        
        plt.axis('off')
        plt.imshow(img, cmap='gray', interpolation='none')
        plt.imshow(heat_map, cmap='jet', alpha=0.4, interpolation='none')
        plt.savefig(os.path.join('heatmap_outputs',category,index_path+'_Predicted heat map.png'),bbox_inches='tight', pad_inches=0, dpi=96)
        plt.close()
        
def multi_plot_heatmap(test_img:np.ndarray, grad, index_path_list):
    ...
    for i in range(num):
        ...
        index_path = index_path_list[i].split(os.path.sep)
        category = index_path[7]
        index_path = "_".join(index_path[7:-1])
        
        for j in range(plus_variable.num_imgs):

            plt.axis('off')
            plt.imshow(img[:,:,j*3:j*3+3], interpolation='none')
            plt.savefig(os.path.join('heatmap_outputs',category,index_path + '_num'+str(j) +'_Orig_Image.png'),bbox_inches='tight', pad_inches=0, dpi=96)
            plt.clf()
            
            
            
            plt.axis('off')
            plt.imshow(img[:,:,j*3:j*3+3], cmap='gray', interpolation='none')
            plt.imshow(heat_map, cmap='jet', alpha=0.4, interpolation='none')
            plt.savefig(os.path.join('heatmap_outputs',category,index_path + '_num'+str(j) +'_Predicted heat map.png'),bbox_inches='tight', pad_inches=0, dpi=96)
            plt.close()
